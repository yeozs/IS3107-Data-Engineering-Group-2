...
CREATE OR REPLACE MODEL `is-data-engineering-project.mlmodel.VThree_LinReg_model`
OPTIONS(model_type='LINEAR_REG', input_label_cols=['resale_price']) AS
SELECT
    resale_price,
    combined_date,
    floor_area_sqm,
...
SELECT *
FROM ML.EVALUATE(
  MODEL `is-data-engineering-project.mlmodel.VThree_LinReg_model`,
...
SELECT *
FROM
    ML.PREDICT(MODEL `is-data-engineering-project.mlmodel.VThree_LinReg_model`,
        (
        SELECT
            resale_price,
            combined_date,
